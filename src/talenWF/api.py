...
class FindTALTask:
    ...
    def __init__(
        ...
        self.filter_base = filter_base - 1 if filter_base is not None else None # make it 0-based
    ...
    def _find_tal_pairs_for_filterpos(self):
        for upstream_base in self.upstream_bases:
            ...
            down_pos_array = findAll(self.sequence, downstream_base, 
                                ...
                                min(len(self.sequence),self.filter_base + ((self.max_dist+1)//2) + 2))
            for up_pos in up_pos_array:
                for down_pos in down_pos_array:
                    ...
                    for spacer_length in range(self.min_spacer, self.max_spacer + 1):
                        ...
                        if sum_tal_lengths >= 2 * self.array_min and sum_tal_lengths <= 2 * self.array_max:
                    ...
                    if ch == upstream_base:
                        ...
                        while upstream_q:
                            ...
                            for spacer_length in range(self.min_spacer, self.max_spacer + 1):
                                ...
                                for tal1_length in range(self.array_min, self.array_max + 1):
                                    tal2_length = sum_tal_lengths - tal1_length
                                    if tal2_length >= self.array_min and tal2_length <= self.array_max:
                                        spacer_start = up_pos + 1 + tal1_length
                                        cut = spacer_start + spacer_length//2
                                        yield self._create_tal_pair(cut, up_pos, j, spacer_length)
    ...
    def _create_tal_pair(self, cut: int, up_pos: int, down_pos: int, spacer_length: int):
        ...
        for tal1_end in tal1_ends:
            ...
            if len(tal1) > self.array_max or len(tal2) > self.array_max \
                or len(tal1) < self.array_min or len(tal2) < self.array_min:
                continue
