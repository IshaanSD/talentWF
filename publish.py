...
def check_and_install_dependencies(python_cmd):
    ...
    if missing_packages:
        ...
        if conda_env:
            print(f"[INFO] Detected conda environment: {conda_env}")
            print("[INFO] Installing missing dependencies via pip...")
        else:
        ...
        try:
            ...
            result = subprocess.run(
                install_cmd, check=True, capture_output=True, text=True, timeout=300
            )
            ...
            if conda_env:
                print(f"[INFO] Try installing manually in conda env:")
                print(f"  {python_cmd} -m pip install {' '.join(missing_packages)}")
                print(
                    f"  Or: conda install -c conda-forge {' '.join(missing_packages)}"
                )
            else:
                print(
                    f"[INFO] Try installing manually: {python_cmd} -m pip install {' '.join(missing_packages)}"
                )
            return False
...
def main():
    ...
    conda_env = os.environ.get("CONDA_DEFAULT_ENV")
    if not conda_env and not Path("conda_env.yaml").exists():
        print(
            "[INFO] Tip: If using conda, activate environment with: conda activate talenWF"
        )
    ...
    if not Path("pyproject.toml").exists():
        print(
            "[ERROR] pyproject.toml not found. Run this script from the package root."
        )
        return 1
    ...
    if choice == "1":
        ...
        if not run_command(
            f"{python_cmd} -m twine upload --repository testpypi dist/*",
        ...
        if not run_command(
            f"{python_cmd} -m twine upload dist/* --verbose", "Uploading to PyPI"
        ):
            return 1
