...
class TestFindTALTask:
    ...
    def test_find_tal_task_upstream_two_in_bounds_far_nofilter(self, output_file):
        ...
        assert sum((out['TAL1 start'] == 2) & (out['TAL1 length'] == 17)) > 0, "Expected result(s) with TAL1 length 17"
        assert sum((out['TAL1 start'] == 2) & (out['TAL2 length'] == 17)) > 0, "Expected result(s) with TAL2 length 17"
        assert sum((out['TAL1 start'] == 2) & (out['Spacer length'] == 17)) > 0, "Expected result(s) with Spacer length 17"
        assert sum((out['TAL1 length'] > 18) | (out['TAL2 length'] > 18) | (out['Spacer length'] > 18)) == 0, "Invalid length(s) found"
